...
contract Plonk4SingleVerifierWithAccessToDNext {
     ...
     * + alpha^6 * ( z_substring[wi] - z_substring[i] + q_substring[i]*(w2[i]*w3[i] -w0[i]*w1[i]) 
    ...
    function batch_evaluate_lagrange_poly_out_of_domain(
        ...
        PairingsBn254.Fr[] memory nums =
            new PairingsBn254.Fr[](poly_nums.length);
        PairingsBn254.Fr[] memory dens =
            new PairingsBn254.Fr[](poly_nums.length + 1); //+1 for Ln
        ...
        PairingsBn254.Fr[] memory partial_products = 
            new PairingsBn254.Fr[](dens.length); // Ln
    ...
    function verify_at_z(
        ...
        /// rhs = r(z)
        PairingsBn254.Fr memory rhs =
            PairingsBn254.copy(proof.linearization_polynomial_at_z);
        // public inputs
        ...
        /// rhs -= "r_complement"
        PairingsBn254.Fr memory quotient_challenge = PairingsBn254.copy(state.alpha);

        PairingsBn254.Fr memory r_permu =
            PairingsBn254.copy(proof.grand_product_at_z_omega);
        //"r_permu"
     ...
     *       + alpha^6 * (w2[z]*w3[z] -w0[z]*w1[z]) *[q_substring]
     *       + alpha^6 * (  
     *          + alpha * ( w0[zw]*(w3[z]*w2[z] +w0[zw] -w0[z]) -w2[zw] )  
     *         ) *[q_substring_r]
     *       + alpha^6 * (alpha^2 * L1[z] - 1) *[z_substring]
    ...
    function reconstruct_d(
        ...
        //(w_0(z) + bata * z + gamma)
        PairingsBn254.Fr memory grand_product_part_at_z =
            PairingsBn254.copy(tmp_fr);
        ...
        grand_product_part_at_z.mul_assign(tmp_fr0);
        tmp_g1 = vk.permutation_commitments[STATE_WIDTH -1].point_mul(grand_product_part_at_z);
        ...
        //+ alpha * L_0(z) + alpha^2 * Ln(z)
        tmp_fr.assign(state.cached_lagrange_eval_Ln);
        tmp_fr.mul_assign(state.alpha);
        tmp_fr.add_assign(state.cached_lagrange_evals[0]);
        tmp_fr.mul_assign(state.alpha);
        grand_product_part_at_z.add_assign(tmp_fr);
        ...
        grand_product_part_at_z.add_assign(out_vu);
        // * [zlookup]
        tmp_g1 = proof.grand_product_lookup_commitment.point_mul(grand_product_part_at_z);
        res.point_add_assign(tmp_g1);
        ...
        // w3[z]*w2[z] (reuse var)
        grand_product_part_at_z.assign(proof.wire_values_at_z[3]);
        grand_product_part_at_z.mul_assign(proof.wire_values_at_z[2]);
        // +w0[zw] -w0[z]
        ...
        grand_product_part_at_z.mul_assign(tmp_fr0);
        // * [q_substring_r]
        tmp_g1 = vk.selector_commitments[STATE_WIDTH +2+2+1].point_mul(grand_product_part_at_z);
        res.point_add_assign(tmp_g1);
        ...
        grand_product_part_at_z.mul_assign(tmp_fr0);
        // * [q_substring]
        tmp_g1 = vk.selector_commitments[STATE_WIDTH +2+2].point_mul(grand_product_part_at_z);
        res.point_add_assign(tmp_g1);
    ...
    function verify_commitments(
        ...
        (PairingsBn254.G1Point memory commitment_aggregation, PairingsBn254.Fr memory out_vu) = reconstruct_d(state, proof, vk);

        /// [F] = [t_0] + z^n * [t_1] + z^{2n} * [t_2] + z^{3n} * [t_3] ...
        PairingsBn254.G1Point memory tmp_g1 = PairingsBn254.P1();

        commitment_aggregation.point_add_assign(proof.quotient_poly_commitments[0]);
        ...
        PairingsBn254.Fr memory aggregation_challenge = PairingsBn254.copy(state.v);
        /// [F] += [D]
        // commitment_aggregation.point_add_assign(d);

        /// [F] += (v^2 +u)[w_0] + v^3[w_1] + (v^4 + uv5)[w_2] + v^5[w_3] + v^6[w_4]
        aggregation_challenge.mul_assign(state.v);
        PairingsBn254.Fr memory tmp_fr0 = PairingsBn254.copy(aggregation_challenge);
        ...
        /// [F] += v^11[qlookup] + v^12[qtable]
        tmp_g1 = vk.selector_commitments[STATE_WIDTH +2].point_mul(aggregation_challenge);
        commitment_aggregation.point_add_assign(tmp_g1);
        aggregation_challenge.mul_assign(state.v);
        tmp_g1 = vk.selector_commitments[STATE_WIDTH +3].point_mul(aggregation_challenge);
        commitment_aggregation.point_add_assign(tmp_g1);

        //([table0] +eta[table1] +eta2[]... +eta5[])
        PairingsBn254.G1Point memory tables_comms_combine =
            PairingsBn254.copy_g1(vk.tables_commitments[0]);
         ...
         *      + v^11 qlookup(z) + v^12 qtable(z) + v^13 table(z)
        ...
        PairingsBn254.Fr memory aggregated_value =
            PairingsBn254.copy(proof.quotient_polynomial_at_z);
        ...
        PairingsBn254.G1Point memory pair_with_x =
            proof.opening_at_z_omega_proof.point_mul(state.u);
    ...
    function verify_initial(
        ...
        TranscriptLibrary.Transcript memory transcript =
            TranscriptLibrary.new_transcript();
        ...
        uint256[] memory lagrange_poly_numbers = new uint256[](vk.num_inputs + tmp);
        for (uint256 i = 0; i < lagrange_poly_numbers.length; i++) {
            lagrange_poly_numbers[i] = i;
        }
        
            PairingsBn254.Fr memory return_zeta_pow_n;
        (state.cached_lagrange_evals, state.cached_lagrange_eval_Ln, return_zeta_pow_n) = 
     ...
     */
    function verify(Proof memory proof, VerificationKey memory vk, bytes32 vkhash)
        internal
        view
        returns (bool)
    {
        ...
        (bool valid, PairingsBn254.Fr memory return_zeta_pow_n) = verify_initial(state, proof, vk, vkhash);
    ...
    Plonk4SingleVerifierWithAccessToDNext
{
    ...
    function vkhash_init (
        ...
        if (string_length == 1024) {
            vk1024hash = sha256( abi.encodePacked(num_inputs,domain_size,vkdata));
        } else if (string_length == 2048) {
            vk2048hash = sha256( abi.encodePacked(num_inputs,domain_size,vkdata));
        } else {
            return false;
        }
    ...
    function multy_verify1024 (
        ...
        console.log("1024");
        bytes32 vkhash = sha256(abi.encodePacked(num_inputs,domain_size,vkdata));
        ...
        for(uint256 i = 0; i < 2; i++){
            vk.selector_commitments[STATE_WIDTH +2 +2 +i] = PairingsBn254.new_g1_checked(
                vkdata[j], vkdata[j+1]
            );
            j += 2;
        }
        ...
        Proof memory proof = deserialize_proof(
            public_inputs,
            serialized_proof
        );
        ...
        (bool res, PairingsBn254.Fr memory return_zeta_pow_n) = verify_initial(state, proof, vk, vkhash);
        if (res == false) {
            return false;
        }
...
console.log(res);
    ...
    function deserialize_proof(
        ...
        //w0(zw)
        proof.wire0_at_z_omega = PairingsBn254.new_fr(
            serialized_proof[j]
        );
        j += 1;
        //z(zw)
        ...
        j += 1;
        //table(zw)
        proof.table_at_z_omega = PairingsBn254.new_fr(
            serialized_proof[j]
        );
        j += 1;
        //w2(zw)
        proof.wire2_at_z_omega = PairingsBn254.new_fr(
            serialized_proof[j]
        );
        j += 1;
        //z_substring(zw)
